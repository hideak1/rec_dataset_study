"""Run BST + SASRec training directly (bypass Jupyter kernel issues)."""
import numpy as np
import pandas as pd
import matplotlib
matplotlib.use('Agg')
...
class TaobaoCTRDataset(Dataset):
    ...
    def _build_samples(self, user_sequences):
        # Pre-sample a large pool of weighted negatives (much faster than per-sample)
        n_users = len(user_sequences)
        pool_size = n_users * self.neg_ratio * 3  # oversample
...
bst_model = BST(n_items=n_items, n_categories=n_categories, embed_dim=32, n_heads=4,
                n_layers=2, max_seq_len=MAX_SEQ_LEN, hidden_dims=[256, 128, 64], dropout=0.2).to(device)
total_params = sum(p.numel() for p in bst_model.parameters())
print(f'\nBST parameters: {total_params:,}')
...
for epoch in range(MAX_EPOCHS):
    ...
    if test_auc > bst_best_auc:
        ...
        patience_counter += 1
    print(f'Epoch {epoch+1:2d}/{MAX_EPOCHS} | Train Loss: {train_loss:.4f} | Test Loss: {test_loss:.4f} | '
          f'Test AUC: {test_auc:.4f} | Test LogLoss: {test_logloss:.4f} | Time: {epoch_time:.1f}s{improved}')
...
class SASRecDataset(Dataset):
    ...
    def _build(self, user_sequences):
        ...
        # Pre-sample a large pool of uniform negatives
...
def train_sasrec_epoch(model, loader, optimizer, device):
    ...
    for batch in loader:
        ...
        mask = (target_ids != 0)  # boolean
        # Use torch.where to avoid NaN * 0 = NaN from padded positions
...
print('\nBST Ablation Results:')
for name, auc in sorted(ablation_results.items(), key=lambda x: x[1], reverse=True):
    print(f'  {name:10s}: AUC = {auc:.4f}')

# ===== Save all results to JSON for notebook injection =====
results = {
    'bst_best_auc': bst_best_auc,
    'bst_history': bst_history,
    'sasrec_best_hr10': sasrec_best_hr10,
    'sasrec_history': sasrec_history,
...
epochs_range = range(1, len(bst_history['train_loss']) + 1)
axes[0].plot(epochs_range, bst_history['train_loss'], 'o-', label='Train', color='#4e79a7')
axes[0].plot(epochs_range, bst_history['test_loss'], 's-', label='Test', color='#e15759')
axes[0].set_xlabel('Epoch'); axes[0].set_ylabel('Loss'); axes[0].set_title('BST: Training & Test Loss'); axes[0].legend()
axes[1].plot(epochs_range, bst_history['test_auc'], 'o-', color='#59a14f', linewidth=2)
axes[1].axhline(0.64, color='red', linestyle='--', alpha=0.7, label='Target AUC=0.64')
axes[1].set_xlabel('Epoch'); axes[1].set_ylabel('AUC'); axes[1].set_title('BST: Test AUC'); axes[1].legend()
axes[2].plot(epochs_range, bst_history['test_logloss'], 'o-', color='#f28e2b', linewidth=2)
axes[2].set_xlabel('Epoch'); axes[2].set_ylabel('LogLoss'); axes[2].set_title('BST: Test LogLoss')
plt.tight_layout()
plt.savefig(os.path.join(NB_DIR, 'bst_training_curves.png'), dpi=150, bbox_inches='tight')
print('Saved bst_training_curves.png')

# SASRec training curves
fig, axes = plt.subplots(1, 3, figsize=(18, 5))
...
print('\n' + '='*70)
print('ALL DONE!')
print(f'BST Best AUC: {bst_best_auc:.4f}')
